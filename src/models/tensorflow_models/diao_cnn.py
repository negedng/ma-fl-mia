...
def get_diao_CNN(
    ...
    layers.append(
        tf.keras.layers.Conv2D(
            hidden_sizes[0], 3, padding="same", input_shape=input_shape, name="conv2d_1"
        )
    )
    ...
    for i in range(len(hidden_sizes) - 1):
        layers.append(
            tf.keras.layers.Conv2D(
                hidden_sizes[i + 1], 3, padding="same", name="conv2d_" + str(i + 2)
            )
        )
        layers.append(
            get_scaler(
                use_scaler, scaler_rate, keep_scaling, name="scaler_" + str(i + 2)
            )
        )
    ...
    layers.extend(
        [
            tf.keras.layers.GlobalAvgPool2D(name="global_average_pooling2d"),
