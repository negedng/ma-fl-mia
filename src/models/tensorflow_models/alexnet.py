    ...
    model = tf.keras.models.Sequential(
        [
            ...
            tf.keras.layers.MaxPool2D(
                pool_size=2, strides=2, padding="valid", name="max_pooling2d_1"
            ),
            tf.keras.layers.Conv2D(
                unit_size * 3, kernel_size=5, padding="same", name="conv2d_2"
            ),
            get_scaler(use_scaler, scaler_rate, keep_scaling, name="scaler_2"),
            tf.keras.layers.ReLU(name="re_lu_2"),
            tf.keras.layers.MaxPool2D(
                pool_size=2, strides=2, padding="valid", name="max_pooling2d_2"
            ),
            tf.keras.layers.Conv2D(
                unit_size * 6, kernel_size=3, padding="same", name="conv2d_3"
            ),
            get_scaler(use_scaler, scaler_rate, keep_scaling, name="scaler_3"),
            tf.keras.layers.ReLU(name="re_lu_3"),
            tf.keras.layers.Conv2D(
                unit_size * 4, kernel_size=3, padding="same", name="conv2d_4"
            ),
            get_scaler(use_scaler, scaler_rate, keep_scaling, name="scaler_4"),
            tf.keras.layers.ReLU(name="re_lu_4"),
            tf.keras.layers.Conv2D(
                unit_size * 4, kernel_size=3, padding="same", name="conv2d_5"
            ),
            get_scaler(use_scaler, scaler_rate, keep_scaling, name="scaler_5"),
            tf.keras.layers.ReLU(name="re_lu_5"),
            tf.keras.layers.MaxPool2D(
                pool_size=2, strides=2, padding="valid", name="max_pooling2d_3"
            ),
