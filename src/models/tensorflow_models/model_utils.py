...
def init_model(unit_size, conf, model_path=None, weights=None, *args, **kwargs):
    model = get_model_architecture(unit_size=unit_size, conf=conf, *args, **kwargs) # maybe you need static_bn?
    if model_path is not None:
        model.load_weights(model_path).expect_partial()
    if weights is not None:
        model.set_weights(weights)
    model.compile(optimizer=get_optimizer(learning_rate=conf['learning_rate']),
                loss=get_loss(),
